...
public class VolumetricCaptureVideoPlayerSequence : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        {
            Debug.Log("No glTF files found.");
        }
        else if (sequence.Length == 1)
        {
            Debug.Log("Single glTF file found. Use Volumetric Capture Video Player instead.");
        }
        else
        {
                ...
                {
                    if (sequence[i].EndsWith(".glb") || sequence[i].EndsWith(".gltf"))
                    {
                        string fileUri = Application.streamingAssetsPath + "/" + sequence[i];
                        await LoadGltfBinaryFromMemory(fileUri, sequence[i]);
